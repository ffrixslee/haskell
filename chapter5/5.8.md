...
c = b 10  
d = c 200  
> Doesn't compile because addOne function requires functions as arguments and but 'b' and 'c' are just values.
...
z). It turns out, the c argument is nowhere in our result, and there’s nothing we need to do with it, so we use the underscore to ignore it. We name the two function arguments by their types and pattern match on the tuple argument. The only way to get the second value of the tuple from the type x to the type z is to use both of the functions furnished to us. If we try the following:  
